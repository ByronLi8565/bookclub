...
and the method body has no obvious handle on *which* connected client invoked
...
- The websocket message handler dispatches RPC inside an `AsyncLocalStorage`
  ...
  callable body runs *within* that context.
...
- `Connection` has server-managed `state` / `setState(...)`. The SDK wraps these
  (index.js ~1043-1063). The only client-driven state path is the *agent* state
